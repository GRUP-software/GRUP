...
export const verifyRecoveryKey = async (req, res) => {
  ...
  try {
    ...
    if (!email || !secretRecoveryKey) {
      return res.status(400).json({ message: "Email and secret recovery key are required" })
    }
...
export const updateSecretRecoveryKey = async (req, res) => {
  ...
  try {
    ...
    if (!currentPassword || !newSecretRecoveryKey) {
      return res.status(400).json({ message: "Current password and new secret recovery key are required" })
    }
    ...
    res.status(500).json({ message: "Failed to update secret recovery key", error: err.message })
  }
}
...
export const requestRecoveryKeyReset = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({ message: "User not found with provided email and phone number" })
    }
    ...
    res.status(500).json({ message: "Failed to submit recovery key reset request", error: err.message })
  }
}
...
export const getRecoveryKeyResetRequests = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to get recovery key reset requests", error: err.message })
  }
}
...
export const approveRecoveryKeyReset = async (req, res) => {
  ...
  try {
    ...
    if (user.recoveryKeyResetRequest?.status !== 'pending') {
      return res.status(400).json({ message: "No pending recovery key reset request found" })
    }
    ...
    res.status(500).json({ message: "Failed to approve recovery key reset request", error: err.message })
  }
}
...
export const rejectRecoveryKeyReset = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Failed to reject recovery key reset request", error: err.message })
  }
}
...
export const useTemporaryRecoveryKey = async (req, res) => {
  ...
  try {
    ...
    if (!email || !temporaryKey || !newRecoveryKey) {
      return res.status(400).json({ message: "Email, temporary key, and new recovery key are required" })
    }

    if (newRecoveryKey.length < 8) {
      return res.status(400).json({ message: "New recovery key must be at least 8 characters long" })
    }
    ...
    if (user.recoveryKeyResetRequest?.status !== 'approved' || 
        user.recoveryKeyResetRequest?.temporaryKey !== temporaryKey) {
      return res.status(400).json({ message: "Invalid temporary recovery key" })
    }

    // Check if temporary key has expired
    if (user.recoveryKeyResetRequest?.expiresAt < new Date()) {
      return res.status(400).json({ message: "Temporary recovery key has expired" })
    }
    ...
    res.status(500).json({ message: "Failed to update recovery key", error: err.message })
  }
}
