    ...
    try {
        ...
        if (!secretRecoveryKey || secretRecoveryKey.length < 8) {
            return res.status(400).json({
                message:
                    'Secret recovery key must be at least 8 characters long',
            });
        }
...
export const verifyRecoveryKey = async (req, res) => {
    ...
    try {
        ...
        const isKeyValid =
            await user.compareSecretRecoveryKey(secretRecoveryKey);
        if (!isKeyValid) {
            return res
                .status(400)
                .json({ message: 'Invalid secret recovery key' });
        }
...
export const updateSecretRecoveryKey = async (req, res) => {
    ...
    try {
        ...
        if (!currentPassword || !newSecretRecoveryKey) {
            return res.status(400).json({
                message:
                    'Current password and new secret recovery key are required',
            });
        }
...
export const useTemporaryRecoveryKey = async (req, res) => {
    ...
    try {
        ...
        if (!email || !temporaryKey || !newRecoveryKey) {
            return res.status(400).json({
                message:
                    'Email, temporary key, and new recovery key are required',
            });
        }
        ...
        res.status(500).json({
            message: 'Failed to update recovery key',
            error: err.message,
        });
    }
};
