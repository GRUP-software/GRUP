...
export const verifyWhatsAppWebhook = async (req, res) => {
    try {
        ...
        logger.info(
            `🔐 WhatsApp webhook verification attempt: mode=${finalMode}, token=${finalToken}`
        );
        ...
        if (finalMode && finalToken) {
            ...
            if (
                finalMode === 'subscribe' &&
                finalToken === process.env.WHATSAPP_VERIFY_TOKEN
            ) {
            ...
            logger.warn(
                '❌ WhatsApp webhook verification failed: missing parameters'
            );
...
export const handleWhatsAppWebhook = async (req, res) => {
    try {
        ...
        if (
            signature &&
            !whatsappService.verifyWebhookSignature(req.body, signature)
        ) {
