  ...
  try {
    ...
    for (const product of products) {
      ...
      if (newPrice !== product.price) {
        ...
        logger.info(`Dynamic pricing updated for ${product.title}: ₦${product.basePrice} → ₦${newPrice}`)
      }
    }
  } catch (error) {
    ...
    const suggestions = await Product.aggregate([
      ...
      },
      {
        $addFields: {
          ...
          daysOfStock: {
            $cond: [
              { $gt: [{ $arrayElemAt: ["$salesData.avgDailyDemand", 0] }, 0] },
              { $divide: ["$stock", { $arrayElemAt: ["$salesData.avgDailyDemand", 0] }] },
              999,
            ],
          },
        },
      ...
      {
        $match: {
          $or: [{ stock: { $lte: "$lowStockThreshold" } }, { daysOfStock: { $lt: 14 } }],
        },
      },
      {
        $project: {
          ...
          priority: {
            $cond: [
              { $lte: ["$daysOfStock", 7] },
              "URGENT",
              { $cond: [{ $lte: ["$daysOfStock", 14] }, "HIGH", "MEDIUM"] },
            ],
          },
        },
      },
    ...
    for (const user of users) {
      ...
      if (recentOrders.length > 0) {
        const rewardAmount = Math.round(
          recentOrders.reduce((sum, order) => sum + order.totalAmount, 0) * rewardPercentage,
        )
        ...
        if (rewardAmount > 0) {
          ...
          logger.info(`Loyalty reward processed: ${user.email} - ₦${rewardAmount} (${loyaltyTier})`)
        }
      }
    }
  } catch (error) {
