        ...
        if (groupBuy) {
          ...
          groupBuyData = {
            ...
            progressPercentage: Math.round(
              (groupBuy.participants.length / groupBuy.minimumViableUnits) *
                100,
            ),
          ...
          sellingUnitsData: product.sellingUnits?.enabled
            ? {
                ...
                options: product.getActiveSellingUnits().map((option) => ({
                  ...option.toObject(),
                  calculatedPrice: product.calculateSellingUnitPrice(
                    option.name,
                  ),
                })),
              }
  ...
  try {
    ...
    if (groupBuy) {
      ...
      groupBuyData = {
        ...
        currentParticipants: groupBuy.participants.length,
        progressPercentage: Math.round(
          (groupBuy.participants.length / groupBuy.minimumViableUnits) * 100,
        ),
        timeRemaining: timeLeft,
        status: groupBuy.status,
        expiresAt: groupBuy.expiresAt,
