    ...
    try {
        ...
        const enrichedProducts = await Promise.all(
            products.map(async (product) => {
                ...
                return {
                    ...product.toObject(),
                    shareLink: `${process.env.NODE_ENV === 'development' ? 'http://localhost:5000' : process.env.BACKEND_URL || 'https://api-staging.grup.com.ng'}/product/${product.slug}?msg=${encodedMessage}`,
                    ...
                    sellingUnitsData: product.sellingUnits?.enabled
                        ? {
                              ...
                              options: product
                                  .getActiveSellingUnits()
                                  .map((option) => ({
                                      ...option.toObject(),
                                      calculatedPrice:
                                          product.calculateSellingUnitPrice(
                                              option.name
                                          ),
                                  })),
                          }
...
export const getProductBySlug = async (req, res) => {
    try {
        ...
        if (groupBuy) {
            const now = new Date();
            const isActive =
                groupBuy.status === 'active' && groupBuy.expiresAt > now;
            const timeLeft = Math.max(0, groupBuy.expiresAt - now);
    ...
    try {
        ...
        if (!title || !description || !basePrice || !stock) {
            return res.status(400).json({
                success: false,
                message:
                    'Title, description, base price, and stock are required fields',
            });
        }
        ...
        if (sellingUnits) {
            try {
                productData.sellingUnits =
                    typeof sellingUnits === 'string'
                        ? JSON.parse(sellingUnits)
                        : sellingUnits;
            } catch (error) {
                return res.status(400).json({
                    success: false,
...
export const getSellingUnitPrice = async (req, res) => {
    try {
        ...
        const option = product.sellingUnits.options.find(
