...
export const trackOrderByNumber = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({ trackingNumber })
      ...
      .populate("items.groupbuyId", "status progressPercentage participantCount minimumViableUnits unitsSold")
...
export const lookupOrderForStaff = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error looking up order", error: err.message })
  }
}
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    if (status === "packaged" && !order.estimatedFulfillmentTime) {
      order.estimatedFulfillmentTime = new Date(Date.now() + 2 * 60 * 60 * 1000) // 2 hours from now
    }
...
export const handleFulfillmentChoice = async (req, res) => {
  try {
    ...
    if (!["pickup", "delivery"].includes(choice)) {
      return res.status(400).json({ message: "Invalid choice. Must be 'pickup' or 'delivery'" })
    }
    ...
    res.status(500).json({ message: "Error handling fulfillment choice", error: err.message })
  }
}
