...
const formatCartItems = (cartItems) => {
  return cartItems.map((item) => {
    ...
    if (item.sellingUnit && item.product.sellingUnits?.enabled) {
      ...
      itemPrice =
        item.sellingUnit.pricePerUnit || item.unitPrice || item.product.price;
      displayInfo = {
    ...
    if (existingItemIndex > -1) {
      ...
      if (cart.items[existingItemIndex].sellingUnit) {
        const baseUnitQuantity =
          Number(cart.items[existingItemIndex].sellingUnit.baseUnitQuantity) ||
          0;
  ...
  try {
    ...
    if (validQuantity <= 0) {
      ...
      if (cart.items[itemIndex].sellingUnit && product.sellingUnits?.enabled) {
        ...
        const baseUnitQuantity =
          Number(cart.items[itemIndex].sellingUnit.baseUnitQuantity) || 0;
...
export const removeFromCart = async (req, res) => {
  try {
    ...
    cart.items = cart.items.filter((item) => {
      ...
      const sellingUnitMatch =
        (item.sellingUnit?.optionName || null) === (sellingUnitName || null);
