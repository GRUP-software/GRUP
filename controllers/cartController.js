...
import {
    ...
    calculateOriginalUnitPrice,
} from '../utils/sellingUnitHelper.js';
...
const calculateSellingUnitPrice = (product, sellingUnit) => {
    ...
    const baseUnitPrice =
        fullProductBaseUnits > 0
            ? product.price / fullProductBaseUnits
            : product.price;
...
const cleanupCartItems = async (cart) => {
    ...
    if (removedCount > 0) {
        console.log(
            `Cleaned up ${removedCount} cart items with null products for user: ${cart.user}`
        );
        await cart.save();
    }
...
export const addToCart = async (req, res) => {
    try {
        ...
        const existingItemIndex = cart.items.findIndex((item) => {
            ...
            const variantMatch =
                normalizedRequestVariant === normalizedItemVariant;

            // Normalize selling unit comparison - treat null and undefined as equivalent
            const normalizedRequestSellingUnit =
                sellingUnitData?.optionName || null;
            const normalizedItemSellingUnit =
                item.sellingUnit?.optionName || null;
            const sellingUnitMatch =
                normalizedRequestSellingUnit === normalizedItemSellingUnit;
        ...
        let updatedCart = await Cart.findOne({ user: userId }).populate(
            'items.product'
        );
        ...
        if (updatedCart) {
            ...
            // Re-populate after cleanup
            updatedCart = await Cart.findOne({ user: userId }).populate(
                'items.product'
            );
        }
...
export const updateCartQuantity = async (req, res) => {
    try {
        ...
        if (validQuantity <= 0) {
            ...
            if (
                cart.items[itemIndex].sellingUnit &&
                product.sellingUnits?.enabled
            ) {
                // Recalculate selling unit price
                const sellingUnitPrice = calculateSellingUnitPrice(
                    product,
                    cart.items[itemIndex].sellingUnit
                );
                cart.items[itemIndex].unitPrice = sellingUnitPrice;
                cart.items[itemIndex].sellingUnit.pricePerUnit =
                    sellingUnitPrice;

                // Update total base units
                const baseUnitQuantity =
                    Number(
                        cart.items[itemIndex].sellingUnit.baseUnitQuantity
                    ) || 0;
                cart.items[itemIndex].sellingUnit.totalBaseUnits =
                    baseUnitQuantity * validQuantity;
            } else {
