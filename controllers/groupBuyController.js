...
export const getActiveGroupBuys = async (req, res) => {
    try {
        ...
        const groupBuys = await GroupBuy.find({
            ...
            .populate(
                'productId',
                'title price images description unitTag slug'
            )
            .sort({ createdAt: -1 })
            .limit(limit * 1)
            .skip((page - 1) * limit);
        ...
        res.status(500).json({
            message: 'Error fetching group buys',
            error: error.message,
        });
    }
};
...
export const getGroupBuyByProduct = async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: 'Error fetching group buy',
            error: error.message,
        });
    }
};
            ...
            if (userId) {
                ...
                if (userParticipation) {
                    groupBuyObj.userParticipation = {
                        ...
                        userPurchaseCount:
                            userParticipation.paymentHistories.length,
                    };
                }
            }
    ...
    try {
        ...
        const userGroupBuys = groupBuys.map((groupBuy) => {
            ...
            return {
                ...
                userJoinedAt: userParticipation?.joinedAt,
                userPurchaseCount:
                    userParticipation?.paymentHistories.length || 0,
...
export const getGroupBuyById = async (req, res) => {
    try {
        ...
        const groupBuy = await GroupBuy.findById(id)
            .populate(
                'productId',
                'title price images category description slug'
            )
...
export const reviewGroupBuy = async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: 'Error reviewing group buy',
            error: error.message,
        });
    }
};
    ...
    try {
        ...
        for (const participant of groupBuy.participants) {
            try {
                ...
                logger.info(
                    `💰 Refunded ₦${participant.amount} to user ${userId}`
                );
            } catch (error) {
                logger.error(
                    `❌ Error processing refund for user ${userId}:`,
                    error
                );
...
const updateOrderStatusForGroupBuy = async (groupBuy, newStatus) => {
    try {
        ...
        for (const order of relatedOrders) {
            ...
            if (orderUpdated) {
                ...
                const statusMessages = {
                    secured:
                        'Group buy has been secured and is ready for processing!',
        ...
        logger.error(
            `❌ Error updating orders for GroupBuy status change:`,
            error
        );
    }
};
...
export const updateGroupBuyMVU = async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: 'Error updating group buy MVU',
            error: error.message,
        });
    }
};
                ...
                for (const order of relatedOrders) {
                    ...
                    if (userPhone) {
                        ...
                        const groupBuyItem = order.items.find(
                            (item) =>
                                item.groupbuyId &&
                                item.groupbuyId.toString() ===
                                    groupBuy._id.toString()
                        );
