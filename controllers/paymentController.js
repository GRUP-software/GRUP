    ...
    try {
      ...
      if (io && groupBuys.length > 0) {
        ...
        if (user) {
          ...
          const product = await Product.findById(groupBuys[0].productId).select(
            "title",
          );
...
const processPartialWalletPayment = async (
  ...
  try {
    ...
    if (walletUse <= 0 || walletUse > paymentHistory.amount) {
      return res
        .status(400)
        .json({ message: "Invalid wallet use amount for partial payment" });
    }
  ...
  try {
    ...
    for (const item of cart.items) {
      ...
      if (item.sellingUnit && product.sellingUnits?.enabled) {
        itemPrice =
          item.sellingUnit.pricePerUnit || item.unitPrice || product.price;
      }
