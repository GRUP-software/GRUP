...
/**
 * Test Script for Group Buy Status Notifications
 * 
 * This script tests the group buy status notification system
 */
...
class GroupBuyNotificationTester {
  constructor() {
    this.testResults = [];
    this.errors = [];
  }
  ...
  async testGroupBuyStatusNotification(user) {
    logger.info('\n🧪 Testing Group Buy Status Notification...');
    ...
    try {
      ...
      if (result) {
        logger.info('✅ Group buy status notification sent successfully');
        this.testResults.push('Group Buy Status Notification: PASS');
      } else {
        throw new Error('Notification service returned null');
      }
    } catch (error) {
      logger.error('❌ Group buy status notification error:', error);
      this.testResults.push('Group Buy Status Notification: FAIL');
      this.errors.push(`Group buy status notification error: ${error.message}`);
    }
  }
  ...
  printTestResults() {
    logger.info('\n' + '='.repeat(60));
    logger.info('📊 GROUP BUY NOTIFICATION TEST RESULTS');
    ...
    if (passedTests === totalTests) {
      logger.info('🎉 All tests passed! Group buy notifications are working correctly.');
    } else {
  ...
  async runAllTests() {
    logger.info('🚀 Starting Group Buy Notification Tests...');
    ...
    try {
      ...
      await this.testGroupBuyStatusNotification(user);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const tester = new GroupBuyNotificationTester();
