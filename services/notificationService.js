...
class NotificationService {
    ...
    async sendEmailNotification(userId, template, data) {
        try {
            ...
            if (!emailService || !emailService.isConfigured) {
                logger.warn(
                    `📧 Email service not available for template: ${template}`
                );
                return {
                    success: false,
            ...
            switch (template) {
                ...
                case 'admin_action_notification':
                    ...
                    htmlContent =
                        emailService.generateAdminActionNotificationEmail({
                            ...data,
                            customerName: user.name,
                        });
                    break;
            ...
            try {
                ...
                logger.error(
                    `❌ Email service error for user ${userId}:`,
                    emailError
                );
    ...
    async notifyAdminRefundProcessed(
        userId,
        amount,
        reason,
        orderId,
