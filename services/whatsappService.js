...
class WhatsAppService {
    ...
    async processWebhook(webhookData) {
        try {
            ...
            if (messageType === 'text') {
                return await this.handleTextResponse(
                    phoneNumber,
                    message.text.body
                );
            }
    ...
    async handleButtonResponse(phoneNumber, buttonReply) {
        try {
            ...
            if (buttonId === 'pickup' || buttonId === 'delivery') {
                ...
                if (!order) {
                    logger.warn(
                        `Order not found for phone number: ${phoneNumber}`
                    );
                    return {
                        success: false,
                ...
                const pickupLocation =
                    'Main Store Location - 123 Commerce Street'; // Make this configurable
            ...
            if (
                ...
                logger.warn(
                    `Phone number mismatch for order ${trackingNumber}`
                );
                return {
                    success: false,
    ...
    async handleTextResponse(phoneNumber, text) {
        try {
            ...
            if (!trackingMatch) {
                return {
                    success: false,
                    error: 'No tracking number found in message',
                };
            }
