    ...
    const fetchGroupProgress = async () => {
        try {
            const response = await fetch(
                `/api/group/group-status/${product._id}`
            );
            if (response.ok) {
        ...
        if (onAddToCart) {
            const sellingUnitData = selectedSellingUnit
                ? {
                      ...
                      baseUnitName:
                          product.sellingUnitsData?.baseUnitName || 'unit',
                      pricePerUnit: selectedSellingUnit.calculatedPrice,
                      totalBaseUnits:
                          selectedSellingUnit.baseUnitQuantity * quantity,
                  }
                : null;
                ...
                {hasSellingUnits && (
                    <div className="space-y-2 mb-4">
                        ...
                        <div className="relative">
                            ...
                            >
                                {activeSellingUnits.map(
                                    (option: any, index: number) => {
                                        const optionPrice =
                                            option.calculatedPrice || 0;
                    ...
                    <div className="space-y-3 mb-4">
                        <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-green-600">
                                {Math.round(groupProgress.progressPercentage)}%
                                complete
                            </span>
                        </div>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                            <div
                                ...
                                style={{
                                    width: `${groupProgress.progressPercentage}%`,
                                }}
                            ></div>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-600">
                            <div className="flex items-center gap-1">
                                <Users className="h-3 w-3" />
                                <span>
                                    {groupProgress.participantCount} joined
                                </span>
                            </div>
                            <div className="flex items-center gap-1">
