...
export const processExpiredGroupBuys = async () => {
    try {
        ...
        logger.info(
            `Found ${expiredGroupBuys.length} expired group buys to process`
        );
        ...
        for (const groupBuy of expiredGroupBuys) {
            try {
                ...
                logger.info(
                    `  Progress: ${groupBuy.unitsSold}/${groupBuy.minimumViableUnits} (${progressPercentage}%)`
                );
                ...
                if (groupBuy.unitsSold >= groupBuy.minimumViableUnits) {
                    ...
                    logger.info(
                        `✅ GroupBuy ${groupBuy._id} marked as secured`
                    );
                } else {
                    ...
                    const partialOrderResult =
                        await processPartialOrderRefunds(groupBuy);
                    if (partialOrderResult.partialOrdersProcessed > 0) {
                        partialOrderCount +=
                            partialOrderResult.partialOrdersProcessed;
                    }

                    manualReviewCount++;
                    logger.info(
                        `GroupBuy ${groupBuy._id} moved to manual review`
                    );
                }
                ...
                logger.error(
                    `Error processing GroupBuy ${groupBuy._id}:`,
                    error
                );
            }
        }
    ...
    try {
        ...
        for (const order of relatedOrders) {
            ...
            if (orderUpdated) {
                ...
                logger.info(
                    `✅ Updated order ${order.trackingNumber} - group buy secured`
                );
            }
        }
    } catch (error) {
        logger.error(
            `❌ Error updating orders for successful GroupBuy ${groupBuy._id}:`,
            error
        );
    }
};
    ...
    try {
        ...
        for (const order of relatedOrders) {
            try {
                ...
                logger.info(
                    `💰 Partial refund processed: ₦${refundAmount} for order ${order.trackingNumber}`
                );
                ...
                logger.error(
                    `❌ Error processing partial refund for order ${order.trackingNumber}:`,
                    error
                );
                result.errors.push({
                    orderId: order._id,
        ...
        logger.info(
            `✅ Processed ${result.partialOrdersProcessed} partial refunds totaling ₦${result.totalRefunded}`
        );
        return result;
    } catch (error) {
    ...
    try {
        ...
        for (const order of relatedOrders) {
            ...
            if (orderUpdated) {
                ...
                await order.save();
                logger.info(
                    `Updated order ${order.trackingNumber} for expired GroupBuy`
                );
            }
        }
    } catch (error) {
        logger.error(
            `Error updating related orders for GroupBuy ${groupBuy._id}:`,
            error
        );
    }
};
