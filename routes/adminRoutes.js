    ...
    async (req, res) => {
        try {
            ...
            const order = await Order.findOne({ trackingNumber })
                ...
                .populate(
                    'items.groupbuyId',
                    'status unitsSold minimumViableUnits'
                )
                .populate(
                    'paymentHistoryId',
                    'referenceId flutterwaveReference status'
                );
...
router.patch('/orders/:orderId/status', verifyAdminToken, async (req, res) => {
    try {
        ...
        if (estimatedFulfillmentTime)
            order.estimatedFulfillmentTime = new Date(estimatedFulfillmentTime);
        ...
        if (currentStatus && currentStatus !== previousStatus) {
            order.progress.push({
                ...
                message:
                    message ||
                    `Order status updated to ${currentStatus} by admin`,
                timestamp: new Date(),
            });
        }
...
router.post('/process-missing-bonuses', verifyAdminToken, async (req, res) => {
    try {
        const { processReferralBonus, processAllMissingBonuses } = await import(
            '../utils/referralBonusService.js'
        );
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            console.log(
                `🔧 Admin requested bonus processing for user: ${userId}`
            );
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            logger.info(
                `📦 Admin ${adminName} cancelled order ${trackingNumber}: ${reason}`
            );
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            logger.info(
                `💰 Admin ${adminName} processed refund for order ${trackingNumber}: ₦${amount} - ${reason}`
            );
...
router.post(
    '/orders/:trackingNumber/schedule-delivery',
    verifyAdminToken,
    async (req, res) => {
        try {
            const { trackingNumber } = req.params;
            ...
            logger.info(
                `🚚 Admin ${adminName} scheduled delivery for order ${trackingNumber}: ${deliveryInfo}`
            );
...
router.post(
    '/orders/:trackingNumber/ready-pickup',
    verifyAdminToken,
    async (req, res) => {
        try {
            const { trackingNumber } = req.params;
                ...
                if (userPhone) {
                    ...
                    if (groupBuyId) {
                        ...
                        const groupBuyItem = order.items.find(
                            (item) =>
                                item.groupbuyId &&
                                item.groupbuyId.toString() === groupBuyId
                        );

                        if (groupBuyItem) {
                            const groupBuyDetails = {
                                totalAmount:
                                    groupBuyItem.price * groupBuyItem.quantity,
                                itemCount: groupBuyItem.quantity,
                                productName:
                                    groupBuyItem.product?.title || 'Product',
                            };
                            ...
                            logger.warn(
                                `⚠️ GroupBuy ${groupBuyId} not found in order ${trackingNumber}`
                            );
                        }
                    } else {
                        logger.warn(
                            `⚠️ No GroupBuy ID provided for admin-triggered WhatsApp message`
                        );
                    }

                    if (whatsappResult.success) {
                        logger.info(
                            `📱 WhatsApp fulfillment choice message sent for GroupBuy ${groupBuyId} (Order: ${trackingNumber})`
                        );
                    } else {
                        logger.warn(
                            `⚠️ Failed to send WhatsApp message for GroupBuy ${groupBuyId}: ${whatsappResult.error}`
                        );
                    }
                } else {
                    logger.warn(
                        `⚠️ No phone number found for order ${trackingNumber}`
                    );
                }
            } catch (whatsappError) {
            ...
            logger.info(
                `📦 Admin ${adminName} marked order ${trackingNumber} ready for pickup at ${pickupLocation}`
            );
...
router.get(
    '/orders/:trackingNumber/whatsapp',
    verifyAdminToken,
    getOrderWhatsAppMessages
);
