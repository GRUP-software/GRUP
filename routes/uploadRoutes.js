...
import { uploadToCloudinary, uploadSingleToCloudinary } from '../middleware/cloudinaryUpload.js';
...
// Upload single image
router.post('/single', verifyToken, uploadSingleToCloudinary('image'), async (req, res) => {
  try {
    ...
    const uploadedImage = await UploadedImage.create({
      ...
      description: req.body.description || `Uploaded image: ${req.cloudinaryResult.originalName}`,
...
router.post('/multiple', verifyToken, uploadToCloudinary('images', 10), async (req, res) => {
  try {
    ...
    for (const result of req.cloudinaryResults) {
      const uploadedImage = await UploadedImage.create({
        ...
        uploadedBy: req.user.email || 'admin',
        description: req.body.description || `Uploaded image: ${result.originalName}`,
        tags: req.body.tags ? req.body.tags.split(",").map((tag) => tag.trim()) : [],
      });
...
router.delete('/:publicId', verifyToken, async (req, res) => {
  try {
    ...
    const dbResult = await UploadedImage.findOneAndDelete({ filename: publicId });
...
router.post('/admin/single', verifyAdminToken, uploadSingleToCloudinary('image'), async (req, res) => {
  try {
    if (!req.cloudinaryResult) {
      return res.status(400).json({
        success: false,
...
router.post('/admin/multiple', verifyAdminToken, uploadToCloudinary('images', 10), async (req, res) => {
  try {
    if (!req.cloudinaryResults || req.cloudinaryResults.length === 0) {
      return res.status(400).json({
        success: false,
