...
export const whatsappConfig = {
  ...
  templates: {
    ...
    confirmation: {
      pickup: {
        message: '✅ Pickup confirmed! Please visit:\n📍 {pickupLocation}\n📞 Bring your tracking number: #{trackingNumber}'
      },
      delivery: {
        message: '✅ Delivery confirmed! We\'ll deliver to your address within 24 hours.\n📞 Tracking: #{trackingNumber}\n💳 Delivery fee: ₦500'
      }
    },
    help: {
      message: 'Need help with order #{trackingNumber}?\n\nReply with:\n• "pickup" to collect from store\n• "delivery" for home delivery\n\nOr use the buttons in the previous message.'
    }
  },
...
export const getPickupLocation = (locationId = 'default') => {
  const location = whatsappConfig.pickupLocations.locations.find(loc => loc.id === locationId)
  return location ? location.name + ' - ' + location.address : whatsappConfig.pickupLocations.default
}
