...
export const processWalletDebit = async (userId, amount, reason, description, metadata = {}) => {
  if (FORCE_ATOMIC) {
    return processWalletDebitAtomic(userId, amount, reason, description, metadata);
  }
  
  const supportsTransactions = await checkTransactionSupport();
  
  if (supportsTransactions) {
    return processWalletDebitWithTransaction(userId, amount, reason, description, metadata);
  } else {
    return processWalletDebitAtomic(userId, amount, reason, description, metadata);
  }
};
...
export const processWalletCredit = async (userId, amount, reason, description, metadata = {}) => {
  if (FORCE_ATOMIC) {
    return processWalletCreditAtomic(userId, amount, reason, description, metadata);
  }
  
  const supportsTransactions = await checkTransactionSupport();
  
  if (supportsTransactions) {
    return processWalletCreditWithTransaction(userId, amount, reason, description, metadata);
  } else {
    return processWalletCreditAtomic(userId, amount, reason, description, metadata);
  }
};
...
const processWalletDebitWithTransaction = async (userId, amount, reason, description, metadata = {}) => {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    console.log(`💳 Processing wallet debit (transaction): User ${userId}, Amount: ₦${amount}, Reason: ${reason}`);
    ...
    console.log(`✅ Wallet debit successful: ₦${amount} deducted from user ${userId}`);
...
const processWalletDebitAtomic = async (userId, amount, reason, description, metadata = {}) => {
  try {
...
const processWalletCreditWithTransaction = async (userId, amount, reason, description, metadata = {}) => {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    console.log(`💳 Processing wallet credit (transaction): User ${userId}, Amount: ₦${amount}, Reason: ${reason}`);
    ...
    await session.commitTransaction();
    console.log(`✅ Wallet credit successful: ₦${amount} added to user ${userId}`);
...
const processWalletCreditAtomic = async (userId, amount, reason, description, metadata = {}) => {
  try {
...
export const getWalletDataWithAggregation = async (userId, page = 1, limit = 20) => {
  try {
    ...
    const wallet = walletData[0] || { balance: 0, totalTransactions: 0, referralEarnings: 0, totalEarned: 0, totalSpent: 0, netBalance: 0 };
