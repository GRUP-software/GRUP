...
export const checkReferralBonusEligibility = async (userId) => {
    try {
        ...
        const totalBonusAmountGiven = referralTransactions.reduce(
...
export const processReferralBonus = async (userId) => {
    try {
        ...
        user.referralStats.totalBonusesEarned =
            (user.referralStats.totalBonusesEarned || 0) + bonusAmount;
        await user.save();
...
export const addReferral = async (referrerId, referredUserId) => {
    try {
        ...
        referrer.referralStats.nextBonusAt =
            Math.ceil((referrer.referralStats.totalReferrals + 1) / 3) * 3;
...
export const processAllMissingBonuses = async () => {
    try {
        ...
        for (const user of users) {
            try {
                const eligibility = await checkReferralBonusEligibility(
                    user._id
                );
                ...
                console.error(
                    `❌ Error processing bonus for user ${user.email}:`,
                    error
                );
            }
        }
...
export const getReferralStats = async (userId) => {
    try {
        ...
        const referralsNeeded = Math.max(
            0,
            nextBonusMilestone - totalReferrals
        );
        const progressPercentage =
            totalReferrals > 0 ? ((totalReferrals % 3) / 3) * 100 : 0;
        ...
        const canReceiveBonus =
            totalBonusesEarned_should > bonusesActuallyGiven;
