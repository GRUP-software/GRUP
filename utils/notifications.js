...
export const notifyGroupSecured = async (group) => {
  try {
    ...
    for (const order of orders) {
      ...
      console.log(`Notification sent to ${order.user.name}: Group secured for order ${order._id}`)
    }
  } catch (err) {
...
export const notifyOrderReady = async (orderId) => {
  try {
    ...
    if (order && order.allGroupsSecured) {
      ...
      console.log(`Order ${orderId} is ready for processing - all groups secured`)
