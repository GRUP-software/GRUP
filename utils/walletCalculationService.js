...
export const calculateReferralBonusAmount = (totalReferrals, bonusPerRound = 500, referralsPerRound = 3) => {
...
export const calculateTotalBonusEarned = (totalReferrals, bonusPerRound = 500, referralsPerRound = 3) => {
  return calculateReferralBonusAmount(totalReferrals, bonusPerRound, referralsPerRound);
};

// Calculate referral rounds completed
export const calculateReferralRounds = (totalReferrals, referralsPerRound = 3) => {
  return Math.floor(totalReferrals / referralsPerRound);
};

// Calculate referrals needed for next bonus
export const calculateReferralsNeededForNextBonus = (totalReferrals, referralsPerRound = 3) => {
  const nextBonusMilestone = Math.ceil((totalReferrals + 1) / referralsPerRound) * referralsPerRound;
  return Math.max(0, nextBonusMilestone - totalReferrals);
};

// Calculate progress percentage towards next bonus
export const calculateBonusProgressPercentage = (totalReferrals, referralsPerRound = 3) => {
...
export const getWalletStatistics = async (userId) => {
  try {
    ...
    const referralTransactions = transactions.filter(tx => 
      tx.reason === 'REFERRAL_BONUS' && tx.type === 'credit'
    );
    ...
    const referralEarnings = referralTransactions.reduce((sum, tx) => sum + tx.amount, 0);
    ...
    const referralsNeededForNextBonus = calculateReferralsNeededForNextBonus(totalReferrals);
...
export const calculateWalletOffset = async (userId, totalAmount, requestedWalletUse = null) => {
  try {
    const wallet = await Wallet.findOne({ user: userId });
    if (!wallet) {
      return {
    ...
    const walletUsed = requestedWalletUse !== null 
...
export const getTransactionSummary = async (userId, filters = {}) => {
  try {
    ...
    if (filters.startDate) matchCriteria.createdAt = { $gte: new Date(filters.startDate) };
    if (filters.endDate) {
...
export const validateReferralBonusEligibility = async (userId) => {
  try {
    ...
    const bonusesActuallyGiven = referralTransactions.reduce((sum, tx) => sum + tx.amount, 0);
