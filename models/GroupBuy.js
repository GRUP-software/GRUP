...
groupBuySchema.methods.addOrUpdateParticipant = function (
    userId,
    quantity,
    amount,
    paymentHistoryId
    ...
    console.log(
        `Adding/updating participant: User ${userId}, Quantity: ${quantity}, Amount: ${amount}`
    );
    ...
    if (this.unitsSold >= this.minimumViableUnits && this.status === 'active') {
        ...
        console.log(
            `GroupBuy ${this._id} reached MVU and marked as successful`
        );
...
groupBuySchema.methods.updateStatus = function (
    newStatus,
    adminName,
    notes = ''
) {
    ...
    if (
        ...
        !this.fulfillmentData.processingStartedAt
    ) {
        this.fulfillmentData.processingStartedAt = new Date();
    } else if (
        newStatus === 'packaging' &&
        !this.fulfillmentData.packagingStartedAt
    ) {
        this.fulfillmentData.packagingStartedAt = new Date();
    } else if (
        newStatus === 'ready_for_pickup' &&
        !this.fulfillmentData.readyForPickupAt
    ) {
...
groupBuySchema.pre('save', function (next) {
    ...
    for (const participant of this.participants) {
        ...
        if (!participant.quantity || participant.quantity <= 0) {
            return next(
                new Error('Participant quantity must be a positive number')
            );
        }
        if (participant.amount === undefined || participant.amount < 0) {
            return next(
                new Error('Participant amount must be a non-negative number')
            );
        }
    }

    // Recalculate totals to ensure consistency
    this.unitsSold = this.participants.reduce(
        (total, p) => total + p.quantity,
        0
    );
    this.totalAmountCollected = this.participants.reduce(
    ...
    if (
        this.isExpired() &&
        this.status === 'active' &&
        this.unitsSold < this.minimumViableUnits
    ) {
        this.status = 'manual_review';
...
groupBuySchema.post('save', async function (doc) {
    ...
    if (doc._shouldSendNotifications && doc._notificationData) {
        try {
            ...
            const product = await Product.findById(
                doc._notificationData.productId
            );
            ...
            console.error(
                'Failed to send group buy secured notifications:',
                error
            );
        }
    }
    ...
    if (
        ...
        (doc.status === 'failed' || doc.status === 'manual_review')
    ) {
        try {
            const notificationService = (
                await import('../services/notificationService.js')
            ...
            console.error(
                'Failed to send group buy failed notifications:',
                error
            );
        }
    }
...
groupBuySchema.statics.createWithMVU = function (
    productId,
