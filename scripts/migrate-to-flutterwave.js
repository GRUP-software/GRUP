...
const migrateToFlutterwave = async () => {
  try {
    ...
    // Connect to database
    await mongoose.connect(process.env.MONGODB_URI || process.env.MONGO_URI || 'mongodb://localhost:27017/GRUP');
    ...
    console.log(`📊 Found ${paymentHistories.length} payment records to migrate`);
    ...
    for (const payment of paymentHistories) {
      try {
        ...
        if (payment.flutterwaveReference) {
          console.log(`⏭️  Skipping ${payment._id} - already has flutterwaveReference`);
          skippedCount++;
          continue;
        }
    ...
    console.log(`   Failed: ${paymentHistories.length - migratedCount - skippedCount}`);
