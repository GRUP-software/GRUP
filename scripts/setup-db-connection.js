...
`;
    ...
    try {
        ...
        if (fs.existsSync(envPath)) {
            ...
            console.log(
                '   - Local (with auth): mongodb://username:password@localhost:27017/GRUP'
            );
            console.log(
                '   - Atlas: mongodb+srv://username:password@cluster.mongodb.net/GRUP?retryWrites=true&w=majority'
            );
        } else {
            // Create .env file with template
            fs.writeFileSync(envPath, envTemplate);
            console.log(
                '✅ Created .env file with database connection template'
            );
            console.log('📁 File location:', envPath);
            console.log('\n📝 Next steps:');
            console.log('   1. Open the .env file');
            console.log(
                '   2. Uncomment and update the MONGODB_URI line for your setup'
            );
            console.log('   3. Save the file');
        ...
        console.log(
            '   5. Use: MONGODB_URI=mongodb+srv://username:password@cluster.mongodb.net/GRUP'
        );
    } catch (error) {
